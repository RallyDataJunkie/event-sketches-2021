...
To start with, let's see what rallies are scheduled for the current, active season:
...
s = get_active_season(active_season_url)$rallyEvents$items

colnames(s)
```

Let's preview a couple of those columns:
...
The *startListId* can be used alongside the event ID to look up the startlist for a leg:
...
In the sections dataframe we have one row per section. We notice again the presence of columns that use dataframes to "nest" subdataframes within each row relating to to timing controls and stages associated with each leg.
...
```{r}
controls = do.call(rbind, sections$controls)
...
We can pull also pull stage details from the dataframes contained in the `sections` dataframe from the itinerary lookup:

```{r}
stages = do.call(rbind, sections$stages)
...
We can conveniently obtain the identifier for a particular driver or codriver by searching against their name or three letter code:
...
View the split times for a driver by indexing into the dataframe. The driver `entryId` gives us an index into the driver data:

```{r}
splits$entrySplitPointTimes[splits$entrySplitPointTimes['entryId']==ogierEntryId,]$splitPointTimes
```

We can get a flatter structure over all drivers, converting the elapsed duration in milliseconds to seconds along the way:
...
driver_splits = entry_splits$splitPointTimes %>%
                    bind_rows() %>%
                    mutate(elapsedDurationS = elapsedDurationMs / 1000) %>%
                    select(-elapsedDurationMs)
...
We can cast the data into a wide format, with splits ordered by their distance into the stage:

```{r}
split_cols =  as.character(arrange(splits$splitPoints,distance)$splitPointId)
cols = c('entryId', 'splitPointId', 'elapsedDurationS')

get_driver_splits_wide = function(driver_splits, split_cols){
...
driver_splits_wide =  get_driver_splits_wide(driver_splits, split_cols)
