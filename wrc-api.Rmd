...
An alternative to the `do.call()` approach is to use a tidy approach and use the `dplyr::bind_rows()` function on the `sections$controls` column values via a pipe:
...
### Getting Long Stage Times for Multiple Stages

It will be convenient to be able to retrieve stage times for multiple stages from one function call. One way of achieving that is to create a function to retrieve the details for a single specified stage that can be applied via a `purrr::map()` function call applied to the list of stage IDs we want results data for: 
...
multi_stage_positions_wide = get_multi_stage_positions_wide(multi_stage_times)
...
get_driver_splits = function(entry_splits){
...
driver_splits = get_driver_splits(entry_splits)
